...
//#include <string> // AMEBA PORTING
//Library Starter
void DabbleClass::begin(String bleName)
{
#if defined(ESP32)
	esp32ble.begin(bleName);
#elif defined(ARDUINO_ARCH_AMEBAD)//defined(RTL8722DM) // AMEBA PORTING
    amebaBLE.begin(bleName);
#endif
	init();
}
...
#elif defined(ARDUINO_ARCH_AMEBAD)//defined(RTL8722DM) // AMEBA PORTING
            if(amebaBLE.available())
                Dabble.processInput(amebaBLE.read());
#endif
...
void DabbleClass::processInput()
{
...
#if 1 // Due to AMEBAD BT code has no callback. Uses polling method as workaround.
    bool conn = BLE.connected();
    static bool last_conn;

    isDabbleConnected = conn;//BLE.connected();

    if(conn != last_conn) {
        last_conn = conn;
#ifdef DEBUG
        Serial.print("**** [dabble] connect state = ");
    ...
    {
		isDabbleConnected = true;
		while(amebaBLE.available())
		{
			byte data=amebaBLE.read();
			/*#ifdef DEBUG
			Serial.print(data);
			Serial.print(" ");
        #endif*/
...
void DabbleClass::processFrame(){
  ...
  {
//#ifdef DEBUG
//    Serial.println("**** functionId = BOARDID_REQUEST");
//#endif
